      ...
      const newApplication = await prisma.$transaction(async (tx: any) => {
        ...
        const application = await tx.renewalFormPersonalDetails.create({
          data: {
            ...
            applicationTypeId: (createRequest as any).applicationTypeId ?? undefined,
            categoryId: (createRequest as any).categoryId ?? undefined,
            currentUserId,
      ...
      const renewalRecord = await prisma.$transaction(async (tx: any) => {
        ...
        const newRenewalRecord = await tx.renewalFormPersonalDetails.create({
          data: {
            ...
            presentAddressId,
            permanentAddressId,
            applicationTypeId: (freshLicense as any).applicationTypeId ?? null,
            categoryId: (freshLicense as any).categoryId ?? null,

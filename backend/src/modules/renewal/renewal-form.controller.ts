...
export class RenewalFormController {
  ...
  /**
   * Get all states (helper endpoint)
   */
  @Get('/helpers/states')
  @ApiOperation({
    summary: 'Get available states',
    description: 'Retrieve master data for states',
  })
  @ApiResponse({
    status: 200,
  ...
  async getStates(): Promise<any[]> {
    return this.renewalFormService.getStates();
  }

  /**
   * Get districts by state ID (helper endpoint)
   */
  @Get('/helpers/districts/:stateId')
  @ApiOperation({
    summary: 'Get districts for a state',
    description: 'Retrieve districts in a specific state',
  })
  @ApiParam({
    name: 'stateId',
    type: Number,
  ...
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async getDistrictsByState(@Param('stateId') stateId: string): Promise<any[]> {
    const sId = parseInt(stateId, 10);
    return this.renewalFormService.getDistrictsByState(sId);
  }

  /**
   * Get police stations by division ID (helper endpoint)
   */
  @Get('/helpers/police-stations/:divisionId')
  @ApiOperation({
    summary: 'Get police stations for a division',
    description: 'Retrieve police stations in a specific division',
  })
  @ApiParam({
    name: 'divisionId',
    type: Number,
  ...
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async getPoliceStationsByDivision(
    @Param('divisionId') divisionId: string,
  ): Promise<any[]> {
    const dId = parseInt(divisionId, 10);
    return this.renewalFormService.getPoliceStationsByDivision(dId);
  }

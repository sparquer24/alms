import { IsOptional, isString, IsNotEmpty, IsEnum, IsDateString, IsString } from "class-validator";
import { Sex } from "@prisma/client";
import { ApiProperty, ApiPropertyOptional } from "@nestjs/swagger";

export class CreateRenewalFormDetailsDto {
    ...
    @IsString()
    aadharNumber?: string;
    @ApiPropertyOptional({ example: 'ABCDE1234F', description: 'PAN number as 10-character alphanumeric string' })
    @IsOptional()
    @IsString()
    panNumber?: string;

    @ApiPropertyOptional({ example: true, description: 'Whether the applicant has any disabilities' })
    @IsOptional()

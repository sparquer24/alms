      ...
      try {
        ...
        if (result && result.user) {
          ...
          if (!normalizedRole) {
            ...
            dispatch(setError(''));
            setIsRedirecting(true);
            // Navigate to the first menu item for this role
            // This sets the loginRedirectApplied flag to tell Sidebar to skip localStorage
            const navigated = navigateToDefaultMenu(normalizedRole, router);
            if (!navigated) {
              // Fallback to generic redirect if navigateToDefaultMenu fails

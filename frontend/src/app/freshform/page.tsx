...
export default function FreshFormPage() {
  ...
  useEffect(() => {
    ...
    if (!roleConfig.permissions.canViewFreshForm) {
  ...
  // Filter applications based on fresh forms and search/date filters
  ...
  return (
    <div className="flex h-screen w-full bg-gray-50 font-[family-name:var(--font-geist-sans)]">
      ...
      <main className={`flex-1 p-8 overflow-y-auto ${!showNewForm ? 'ml-[18%] mt-[70px]' : ''}`}>
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-2xl font-bold">Fresh Application Forms</h1>
            <div className="flex space-x-2">
              ...
              {/* Show only Create New when table is empty */}
              {!showNewForm && filteredApplications.length === 0 && (
                <button 
                  className="px-4 py-2 bg-[#6366F1] text-white rounded-md hover:bg-[#4F46E5] flex items-center"
                  onClick={handleNewApplication}

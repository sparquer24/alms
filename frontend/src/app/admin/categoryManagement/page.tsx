...
export default function CategoryManagementPage() {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ['categories'],
    queryFn: () => AdminActionService.getCategories(),
  });

  const createMut = useMutation({
    mutationFn: (data: any) => AdminActionService.createCategory(data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['categories'] }); toast.success('Created'); setShowForm(false); resetForm(); },
    onError: (e: any) => toast.error(e.message || 'Failed'),
  });

  const updateMut = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => AdminActionService.updateCategory(id, data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['categories'] }); toast.success('Updated'); setShowForm(false); setEditing(null); resetForm(); },
    onError: (e: any) => toast.error(e.message || 'Failed'),
  });

  const deleteMut = useMutation({
    mutationFn: (id: number) => AdminActionService.deleteCategory(id),
      ...
      <div style={{ padding: AdminLayout.content.padding, display: 'flex', flexDirection: 'column', gap: AdminSpacing.xl }}>
        <div className='bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden'>
          <div className='bg-[#001F54] text-white px-6 py-8'>
            ...
            <p className='text-blue-100 text-lg'>Manage workflow categories</p>
          </div>
        </div>
          ...
          {isLoading ? <AdminSectionSkeleton /> : (
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {items.map((item: MasterEntity) => (
                  <tr key={item.id}
                    ...
                    <td style={tdStyle}>
                      ...
                      <button onClick={() => { if (confirm('Delete this category?')) deleteMut.mutate(item.id); }}
                ...
                {items.length === 0 && (
                  <tr><td colSpan={5} style={{ ...tdStyle, textAlign: 'center', color: colors.text.secondary }}>No categories configured yet.</td></tr>
                )}
              </tbody>
            </table>
          )}

...
export default function PrintApplicationForm({ application, applicantName }: PrintApplicationFormProps) {
  ...
  return (
    <div className="print-form-container font-serif text-black p-4 w-[210mm] max-w-[210mm] mx-auto bg-white box-border">
      ...
      <div className="text-center font-bold text-base tracking-wide border-2 border-black p-2 mb-4 uppercase">
        Firearms License Application Details
      </div>
      ...
      <table className="print-form-table">
        ...
        <tbody>
          <tr>
            ...
            <td className="value-cell">{formatApplicationType(application.applicationType) || 'Fresh License'}</td>
            <td rowSpan={6} style={{ width: '100px', textAlign: 'center', verticalAlign: 'middle', padding: '2px' }}>
              {photoUrl ? (
                <img
                  src={photoUrl}
                  alt="Applicant Photo"
                  style={{ width: '80px', height: '100px', objectFit: 'cover', border: '1px solid #111', display: 'inline-block' }}
                />
              ) : (
                <div style={{ width: '80px', height: '100px', border: '1px dashed #111', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '8px', margin: '0 auto' }}>
                  Applicant Photo
                </div>
              )}
              <div style={{ fontSize: '8px', marginTop: '2px', fontWeight: 'bold' }}>Applicant Photo</div>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td className="value-cell" style={{ fontFamily: 'monospace' }}>{application.aadharNumber || '—'}</td>
          </tr>
          <tr>
            <td className="label-cell">PAN Number</td>
            <td className="value-cell" style={{ fontFamily: 'monospace' }}>{application.panNumber || '—'}</td>
          ...
          <tr>
            ...
            <td className="value-cell" style={{ fontFamily: 'monospace' }}>{application.acknowledgementNo || '—'}</td>
            <td className="label-cell">Workflow Status</td>
            <td className="value-cell">{application.workflowStatus?.name || application.workflowStatus || '—'}</td>
          </tr>
          <tr>
            <td className="label-cell">Date & Time of Submission</td>
            <td className="value-cell" colSpan={4}>{formatDateTime(application.applicationDate)}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table className="print-form-table">
        ...
        <tbody>
          <tr>
            <td className="label-cell" style={{ width: '20%' }}>Need for License</td>
            <td className="value-cell" style={{ width: '30%' }}>{license.needForLicense || '—'}</td>
            <td className="label-cell" style={{ width: '20%' }}>Previously Applied</td>
            <td className="value-cell" style={{ width: '30%' }}>{history.hasAppliedBefore !== undefined ? (history.hasAppliedBefore ? 'Yes' : 'No') : '—'}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td className="value-cell">{history.hasLicenceSuspended !== undefined ? (history.hasLicenceSuspended ? 'Yes' : 'No') : '—'}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td className="value-cell">{history.hasFamilyLicence !== undefined ? (history.hasFamilyLicence ? 'Yes' : 'No') : '—'}</td>
          </tr>
          <tr>
      ...
      <table className="print-form-table">
        ...
        <tbody>
          <tr>
            ...
            <td className="value-cell">{criminal.isConvicted !== undefined ? (criminal.isConvicted ? 'Yes' : 'No') : '—'}</td>
            <td className="label-cell">Prohibited</td>
            <td className="value-cell">{criminal.isProhibited !== undefined ? (criminal.isProhibited ? 'Yes' : 'No') : '—'}</td>
          </tr>
          <tr>
            <td className="label-cell">Bond Executed</td>
            <td className="value-cell">{criminal.isBondExecuted !== undefined ? (criminal.isBondExecuted ? 'Yes' : 'No') : '—'}</td>
      ...
      {application.documents && application.documents.length > 0 && (
        <div style={{ marginTop: '12px' }}>
          <div className="section-header-row" style={{ border: '1px solid #111', padding: '5px 8px', fontSize: '10px', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '8px' }}>
          ...
          <div className="print-documents-grid">
            {application.documents.map((doc: any, idx: number) => {
              ...
              const isImage = /\.(png|jpe?g|gif|svg)$/i.test(name) || /image/i.test(doc?.contentType);
                ...
                <div key={idx} className="print-doc-card">
                  ...
                  <div className="print-doc-preview-container">
                    {isImage && url ? (
                      ...
                      <div className="print-doc-placeholder">
      ...
      {(() => {
        const rawHistories = Array.isArray(application.workflowHistories)
          ...
          : Array.isArray(application.history)
            ? application.history
            : [];
        const workflowHistories = [...rawHistories].sort((a: any, b: any) =>
        ...
        return (
          <div style={{ marginTop: '16px', pageBreakBefore: workflowHistories.length > 5 ? 'always' : 'auto' }}>
            <div
            ...
            <table className="print-form-table" style={{ pageBreakBefore: 'auto', pageBreakInside: 'auto' }}>
              <thead>
                <tr>
                  <th style={{ width: '5%' }}>#</th>
                ...
                {workflowHistories.map((entry: any, idx: number) => {
                  ...
                  return (
                    <React.Fragment key={entry.id || idx}>
                      <tr>
                        <td style={{ textAlign: 'center', fontWeight: 'normal', fontSize: '8px', border: '1px solid #111', padding: '3px 4px', verticalAlign: 'top' }}>{idx + 1}</td>
                        <td style={{ fontWeight: 'normal', fontSize: '8px', border: '1px solid #111', padding: '3px 4px', verticalAlign: 'top' }}>{userName}</td>
                        <td style={{ fontWeight: 'normal', fontSize: '8px', border: '1px solid #111', padding: '3px 4px', verticalAlign: 'top' }}>{roleName}</td>
                        <td style={{ fontWeight: 'normal', fontSize: '8px', border: '1px solid #111', padding: '3px 4px', verticalAlign: 'top' }}>{actionTaken}</td>
                        <td style={{ fontWeight: 'normal', fontSize: '8px', border: '1px solid #111', padding: '3px 4px', verticalAlign: 'top' }}>{forwardedTo}</td>
            ...
            <div style={{ fontSize: '7px', color: '#555', marginTop: '4px', textAlign: 'right', fontStyle: 'italic' }}>
      ...
      <div className="flex justify-between items-end mt-4 pt-4 text-[9px] font-serif" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '16px' }}>
        <div>

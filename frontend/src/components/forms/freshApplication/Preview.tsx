...
const Preview = () => {
  ...
  const renderBiometricInfo = () => {
    ...
    const biometricData = applicationData.biometricData?.biometricData || applicationData.biometricData || {};

    

    // Check for photo in fileUploads if not in biometricData

    let photoData = biometricData.photo;

    

    if (!photoData && applicationData.fileUploads && Array.isArray(applicationData.fileUploads)) {

      const photoUpload = applicationData.fileUploads.find((f: any) => 

        (f.fileType || '').toUpperCase() === 'PHOTOGRAPH'
    ...
    const hasFingerprints = biometricData.fingerprints && biometricData.fingerprints.length > 0;

    const hasPhoto = !!photoData;

    

    const hasBiometricData = hasFingerprints || hasPhoto;
      ...
      <div className='mb-8 border rounded-lg p-6 bg-white shadow-sm'>
        ...
        <div className='flex justify-between items-center mb-6'>
          ...
          <button
            ...
            onClick={() => handleEdit(FORM_ROUTES.BIOMETRIC_INFO)}

            className='text-blue-600 hover:text-blue-800 flex items-center gap-2 px-3 py-1 rounded-md hover:bg-blue-50'

            title='Edit Biometric Information'
        ...
        {hasBiometricData ? (
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            ...
            <div className='border border-gray-200 rounded-lg p-4 bg-gray-50'>
              ...
              <h4 className='font-medium text-gray-700 border-b pb-2 mb-4'>Fingerprints</h4>

              {hasFingerprints ? (

                <div className='space-y-4'>

                  {biometricData.fingerprints.map((fp: any, index: number) => (

                    <div key={index} className='flex items-center gap-4 bg-white p-3 rounded border border-gray-200'>
                      ...
                      <div className='w-16 h-20 bg-gray-100 rounded overflow-hidden flex-shrink-0 border'>
                        ...
                        {fp.bitmapData ? (
                          ...
                          <img
                            ...
                            src={fp.bitmapData.startsWith('data:') ? fp.bitmapData : `data:image/bmp;base64,${fp.bitmapData}`}
                      ...
                      <div>
                        ...
                        <div className='text-sm font-semibold text-gray-800'>{fp.position}</div>

                        <div className='text-xs text-gray-500 mt-1'>

                          Quality: <span className={fp.quality >= 80 ? 'text-green-600 font-medium' : fp.quality >= 60 ? 'text-yellow-600 font-medium' : 'text-red-600 font-medium'}>{fp.quality}%</span>

                        </div>

                        <div className='text-xs text-gray-400 mt-0.5'>

                          Enrolled: {new Date(fp.enrolledAt).toLocaleDateString()}
            ...
            <div className='border border-gray-200 rounded-lg p-4 bg-gray-50'>
              ...
              <h4 className='font-medium text-gray-700 border-b pb-2 mb-4'>Photograph</h4>

              {hasPhoto ? (

                <div className='flex flex-col items-center'>

                  <div className='w-32 h-40 bg-white rounded-lg overflow-hidden border-2 border-gray-200 shadow-sm mb-3'>
          ...
          <div className='text-center py-8 bg-indigo-50 rounded-lg border border-indigo-200'>
            ...
            <p className='text-indigo-700 font-medium'>No Biometric Data Found</p>

            <p className='text-indigo-600 text-sm mt-1'>

              Please complete the biometric information section

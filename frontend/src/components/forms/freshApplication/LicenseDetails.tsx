...
const LicenseDetails = () => {
   ...
   const removeFile = (indexToRemove: number, isUploaded: boolean = false) => {
	   setForm((prev: any) => {
		   ...
		   if (isUploaded) {
			   const currentUploadedFiles = currentDetail.uploadedFiles || [];
			   const updatedUploadedFiles = currentUploadedFiles.filter((_: any, index: number) => index !== indexToRemove);
		...
		<div className="grid grid-cols-2 gap-8">
			...
			<div className="flex flex-col gap-8">
				...
				<div className="mb-4">
					...
					>
						<option value={0}>{loadingWeapons ? 'Loading weapons...' : 'Select weapon type to add'}</option>
						{weapons.map(weapon => (
								...
								{getLicenseDetail().requestedWeaponIds.map((weaponId: number) => {
									...
									return weapon ? (
										...
										>
											...
											<button
												type="button"
												onClick={() => handleWeaponChange({ target: { value: String(weaponId) } } as any)}
			...
			<div className="flex flex-col gap-8">
				...
				<div className="mb-4">
					...
					<div className="border-2 border-dashed border-gray-300 rounded-lg p-4 hover:border-blue-400 transition-colors">
						...
						{getLicenseDetail().specialClaimsEvidence && getLicenseDetail().specialClaimsEvidence.length > 0 && (
							<div className="mt-4 space-y-2">
								<div className="text-sm font-medium text-gray-700">
								...
								{getLicenseDetail().specialClaimsEvidence.map((file: File, index: number) => (
									<div key={`pending-${index}`} className="flex items-center justify-between p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
										<div className="flex items-center">
						...
						{/* Upload limit info */}
						{(() => {
							const totalFiles = (getLicenseDetail().specialClaimsEvidence?.length || 0) + (getLicenseDetail().uploadedFiles?.length || 0);
							return totalFiles >= 5 && (

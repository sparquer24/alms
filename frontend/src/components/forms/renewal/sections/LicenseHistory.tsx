...
const LicenseHistory: React.FC<{ formData:any; onChange:(e:any)=>void }> = ({ formData, onChange }) => {
		...
		<div className='space-y-4'>
			<div className='space-y-5'>
				<div>
					...
					{yn('hasAppliedBefore', Boolean(formData.hasAppliedBefore), onChange)}

					<div className='mt-3 grid grid-cols-1 md:grid-cols-2 gap-4'>
						<Input label='Date of Application' type='date' name='applicationDate' value={formData.applicationDate || ''} onChange={onChange} />
						<Input label='To which authority' name='authorityAppliedTo' value={formData.authorityAppliedTo || ''} onChange={onChange} placeholder='Enter authority' />
						<Select
							label='Result'
				...
				<div>
					...
					{yn('licenseRevokedOrSuspended', Boolean(formData.licenseRevokedOrSuspended), onChange)}
					<div className='mt-3 grid grid-cols-1 md:grid-cols-2 gap-4'>
						<Input label='By which authority' name='revokedByAuthority' value={formData.revokedByAuthority || ''} onChange={onChange} placeholder='Enter authority' />
						<Input label='Reason' name='revokedReason' value={formData.revokedReason || ''} onChange={onChange} placeholder='Enter reason' />
					</div>
				</div>

				<div>
					<p className='text-sm'>(c) Any member of the family holds a license</p>
					{yn('familyMemberHasLicense', Boolean(formData.familyMemberHasLicense), onChange)}
					<div className='mt-3 grid grid-cols-1 md:grid-cols-2 gap-4'>
						<Input label='Name' name='familyMemberName' value={formData.familyMemberName || ''} onChange={onChange} placeholder='Enter name' />
						<Input label='License Number' name='familyLicenseNumber' value={formData.familyLicenseNumber || ''} onChange={onChange} placeholder='Enter license number' />
					</div>
				...
				<div>
					...
					{yn('hasSafeCustody', Boolean(formData.hasSafeCustody), onChange)}
					<TextArea label='If Yes details thereof' name='safeCustodyDetails' value={formData.safeCustodyDetails || ''} onChange={onChange} rows={3} placeholder='Enter details' />
				</div>

				<div>
					<p className='text-sm'>(e) The applicant has undergone training as specified under rule 10</p>
					{yn('hasTrainingUnderRule10', Boolean(formData.hasTrainingUnderRule10), onChange)}
					<TextArea label='If Yes details thereof' name='trainingDetails' value={formData.trainingDetails || ''} onChange={onChange} rows={3} placeholder='Enter details' />
				</div>
			</div>
		</div>
	);
